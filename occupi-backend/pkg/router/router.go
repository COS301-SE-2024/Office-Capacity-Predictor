...
func OccupiRouter(router *gin.Engine, db *mongo.Client) {
	...
	api := router.Group("/api")
	{
		...
		api.GET("/user-details", middleware.ProtectedRoute, func(ctx *gin.Context) { handlers.RetrieveUserDetails(ctx, appsession) })
		api.POST("/update-user", middleware.ProtectedRoute, func(ctx *gin.Context) { handlers.UpdateUser(ctx, appsession) })
	}
